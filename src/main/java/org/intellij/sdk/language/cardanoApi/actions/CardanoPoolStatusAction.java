...
public class CardanoPoolStatusAction extends AnAction {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        // Fetch pool status in the background
        new Task.Backgroundable(project, "Fetching pool status") {
            @Override
            public void run(@NotNull ProgressIndicator indicator) {
                indicator.setText("Fetching status for pool ID...");
                try {
                    fetcher.fetchPoolStatus(poolId);
                } catch (Exception ex) {
                    throw new RuntimeException("Error fetching pool status: " + ex.getMessage(), ex);
                }
            }

            @Override
            public void onSuccess() {
                Messages.showInfoMessage("Pool status fetched successfully.", "Success");
            }

            @Override
            public void onThrowable(@NotNull Throwable error) {
                Messages.showErrorDialog("Error fetching pool status: " + error.getMessage(), "Error");
            }
        }.queue();
    }
